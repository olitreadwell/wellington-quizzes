...
- Next.js static export (`output: 'export'`; `basePath` is `/wlg-nz-quizzes`
  on GitHub Pages and the root on Vercel, flipped by the `VERCEL` env); no API
  routes, no server runtime. Deployed by `.github/workflows/pages.yml`.
